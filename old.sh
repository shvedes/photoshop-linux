...
install_proton() {
  ...
  if ! gpg --keyserver hkps://keys.openpgp.org --recv-key F3B607488DB35A47 &> /dev/null; then
    echo "$ERORR Something went wrong"
    exit 1
  fi
  
  echo -e "$LOG Downloading proton"
  curl --progress-bar "$proton_url" -o proton-cachyos.tar.zst
  curl --progress-bar "$proton_signature_url" -o proton-cachyos.sig

  echo -e "$LOG Validating checksums"
  if ! gpg --verify proton-cachyos.sig proton-cachyos.tar.zst &> /dev/null; then
    echo -e "$ERORR Valid signature"
    exit 1
  fi
}
...
is_path_exists() {
  if [ -d "$1" ]; then
    ...
    if ask_user "Do you want to ${RED}delete${RESET} previous installation?"; then
      if rm -rf "${1:?}" 2>>./install_log.log; then
